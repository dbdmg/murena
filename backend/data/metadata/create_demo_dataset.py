...
def generate(n: int, seed: int = 42) -> pd.DataFrame:
    ...
    property_type = np.array([_weighted(rng, PROPERTY_TYPES) for _ in range(n)])
    construction_period = np.array(
        [_weighted(rng, CONSTRUCTION_PERIODS) for _ in range(n)]
    )

    period_year_ranges = {
        "Prima del 1919": (1870, 1918),
        "Dal 1919 al 1945": (1919, 1945),
        "Dal 1946 al 1960": (1946, 1960),
        "Dal 1961 al 1980": (1961, 1980),
        "Dal 1981 al 2000": (1981, 2000),
        "Dopo il 2000": (2001, 2023),
    }
    construction_year = np.array(
        [
            rng.integers(
                period_year_ranges[period][0],
                period_year_ranges[period][1] + 1,
            )
    ...
    score_total = (
        score_class + score_plant + score_envelope + score_renewables
    ).round(2)
    ...
    cadastral_units_count = np.where(meta_flags, rng.integers(2, 12, n), 1)
    id_list = [
        str([f"{ids[i]}-U{j}" for j in range(1, int(cadastral_units_count[i]) + 1)])
    ...
    base_rent_rate = np.array([rent_rate_by_type[t] for t in property_type])
    centrality_premium = 0.85 + 0.45 * dist_norm
    rent_noise = rng.normal(1.0, 0.12, n).clip(0.72, 1.35)
    annual_rent = (surface * base_rent_rate * centrality_premium * rent_noise).round(0)
