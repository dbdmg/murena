...
@router.get("/markers-lite")
async def get_map_markers_lite(
    limit: int = 1000,
    min_score: Optional[float] = None,
    max_score: Optional[float] = None,
    city: Optional[str] = None,
    energy_classes: Optional[str] = None,
    min_surface: Optional[float] = None,
    max_surface: Optional[float] = None,
    epoche_costruzione: Optional[str] = None,
    property_types: Optional[str] = None,
    utilizzo_bene: Optional[str] = None,
    vincolo_culturale: Optional[str] = None,
    is_meta_immobile: Optional[bool] = None,
    natura_bene: Optional[str] = None,
    current_user: Any = Depends(deps.get_current_user_optional),
) -> List[Dict[str, Any]]:
    """
    Get lightweight map markers for initial map loading.
    Returns list of dicts directly for performance.
    """
    service = MapService()
    filters = {}
