...
class RunJSONLogger:
        ...
        try:
            ...
            export_data = {
                ...
                "results_summary": {
                    ...
                    "top_10_ids": [b.get("id") for b in results.get("buildings", [])[:10]]
                },
    ...
    def _compute_metrics(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with_ape = sum(1 for b in buildings if b.get("classe_energetica_ape") not in [None, "N/D", ""])
        
        # Count low energy classes (F, G)
        low_energy = sum(1 for b in buildings if b.get("classe_energetica_ape") in ["F", "G"])
    ...
    def _cleanup_old_exports(self, keep: int = 10):
        ...
        try:
            exports = sorted(self.export_dir.glob("*.json"), key=os.path.getmtime, reverse=True)
