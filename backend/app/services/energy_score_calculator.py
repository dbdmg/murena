...
class EnergyScoreCalculator:
    ...
    def __init__(self, ape_data_path: Optional[Path] = None):
        ...
        """
        self.ape_data_path = ape_data_path or Path(__file__).parent.parent.parent / "data" / "FOLDER_META" / "ape_detailed_data.parquet"
    ...
    def load_and_compute(self) -> pd.DataFrame:
        """
        ...
        """
        if self._loaded and self._scores_df is not None:
        ...
        df = df[df["kwh_per_sqm"].notna() & (df["kwh_per_sqm"] > 0) & (df["kwh_per_sqm"] < 1000)]
        ...
        for usage_type in df["destinazione_uso_cod"].dropna().unique():
            ...
            if len(subset) >= 10:  # Need enough samples for meaningful stats
                ...
                logger.info(f"Usage type {usage_type}: mean={subset.mean():.2f}, std={subset.std():.2f}, n={len(subset)}")
        ...
        logger.info(f"Global distribution: mean={all_values.mean():.2f}, std={all_values.std():.2f}, n={len(all_values)}")
    ...
    def get_score_details(self, kwh_per_sqm: float, superficie: float, 
                          usage_type: Optional[float] = None) -> Dict[str, Any]:
        """
        ...
        if usage_type is not None and usage_type in self._distributions:
            dist = self._distributions[usage_type]
            usage_label = USAGE_TYPE_LABELS.get(int(usage_type), f"Tipo {int(usage_type)}")
        else:
            dist = self._global_distribution
        ...
        return {
            ...
            "confronto": {
                ...
                "differenza_percentuale": round(((kwh_per_sqm - dist["mean"]) / dist["mean"]) * 100, 1) if dist and dist["mean"] > 0 else None
            }
        }
...
def calculate_energy_score(kwh_per_sqm: float, usage_type: Optional[float] = None) -> int:
    """
