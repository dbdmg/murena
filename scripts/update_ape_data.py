...
DEFAULT_PARQUET_PATH = BACKEND_DIR / "data" / "FOLDER_META" / "ape_detailed_data.parquet"
...
def calculate_energy_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "consumo_kwh_tot" in df.columns and "superficie" in df.columns:
        ...
        valid_mask = df["kwh_per_sqm"].notna() & (df["kwh_per_sqm"] > 0) & (df["kwh_per_sqm"] < 1000)
        
        # Build distributions per usage type
        distributions = {}
        for usage_type in df.loc[valid_mask, "destinazione_uso_cod"].dropna().unique():
            subset = df.loc[valid_mask & (df["destinazione_uso_cod"] == usage_type), "kwh_per_sqm"]
            if len(subset) >= 10:
                distributions[usage_type] = {
                    "mean": subset.mean(),
                    "std": subset.std()
                }
                print(f"  Usage type {usage_type}: mean={subset.mean():.1f} kWh/m², std={subset.std():.1f}, n={len(subset)}")
        
        # Global distribution as fallback
        all_valid = df.loc[valid_mask, "kwh_per_sqm"]
        global_dist = {"mean": all_valid.mean(), "std": all_valid.std()}
        print(f"  Global: mean={global_dist['mean']:.1f} kWh/m², std={global_dist['std']:.1f}, n={len(all_valid)}")
        ...
        print("Warning: Missing consumo_kwh_tot or superficie columns, skipping energy scores")
