...
def generate_combinations(json_file: str, output_file: str) -> int:
    ...
    BUT only those containing ALL components (no 'None' absence states).
    This is used for the ablation study.
    """
    ...
    for key in keys_order:
        values = data.get(key, [])
        # Mandatory: only the provided values (no None state)
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(base_path, "ablation_composed_queries.csv")
    
    count = generate_combinations(json_path, output_path)
    print(f"Generated {count} ablation query compositions in {output_path}")
